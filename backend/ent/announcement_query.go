...
func (_q *AnnouncementQuery) QueryCreator() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, announcement.CreatorTable, announcement.CreatorColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
func (_q *AnnouncementQuery) WithCreator(opts ...func(*UserQuery)) *AnnouncementQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AnnouncementQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Announcement, error) {
	...
	if query := _q.withCreator; query != nil {
		if err := _q.loadCreator(ctx, query, nodes, nil,
			func(n *Announcement, e *User) { n.Edges.Creator = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AnnouncementQuery) loadCreator(ctx context.Context, query *UserQuery, nodes []*Announcement, init func(*Announcement), assign func(*Announcement, *User)) error {
...
func (_q *AnnouncementQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withCreator != nil {
			_spec.Node.AddColumnOnce(announcement.FieldCreatedBy)
		}
	}
