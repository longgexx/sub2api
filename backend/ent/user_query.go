...
// QueryRedeemStats chains the current query on the "redeem_stats" edge.
func (_q *UserQuery) QueryRedeemStats() *UserRedeemStatQuery {
	query := (&UserRedeemStatClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithRedeemStats tells the query-builder to eager-load the nodes that are connected to
// the "redeem_stats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRedeemStats(opts ...func(*UserRedeemStatQuery)) *UserQuery {
	query := (&UserRedeemStatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRedeemStats = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withRedeemStats; query != nil {
...
func (_q *UserQuery) loadRedeemStats(ctx context.Context, query *UserRedeemStatQuery, nodes []*User, init func(*User), assign func(*User, *UserRedeemStat)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userredeemstat.FieldUserID)
	}
