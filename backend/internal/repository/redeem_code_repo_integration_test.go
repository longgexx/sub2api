...
func (s *RedeemCodeRepoSuite) TestUse() {
	...
	code := &service.RedeemCode{Code: "USE-ME", Type: service.RedeemTypeBalance, Value: 10.0, Status: service.StatusUnused}
	s.Require().NoError(s.repo.Create(s.ctx, code))

	err := s.repo.Use(s.ctx, code.ID, user.ID, 10.0)
...
func (s *RedeemCodeRepoSuite) TestUse_Idempotency() {
	...
	code := &service.RedeemCode{Code: "IDEM-CODE", Type: service.RedeemTypeBalance, Value: 10.0, Status: service.StatusUnused}
	s.Require().NoError(s.repo.Create(s.ctx, code))
...
func (s *RedeemCodeRepoSuite) TestUse_AlreadyUsed() {
	...
	code := &service.RedeemCode{Code: "ALREADY-USED", Type: service.RedeemTypeBalance, Value: 10.0, Status: service.StatusUsed}
	s.Require().NoError(s.repo.Create(s.ctx, code))
...
func (s *RedeemCodeRepoSuite) TestCreateBatch_Filters_Use_Idempotency_ListByUser() {
	...
	s.Require().NoError(err, "GetByCode")
	s.Require().NoError(s.repo.Use(s.ctx, codeB.ID, user.ID, 10.0), "Use")
	err = s.repo.Use(s.ctx, codeB.ID, user.ID, 10.0)
	s.Require().Error(err, "Use expected error on second call")
	s.Require().ErrorIs(err, service.ErrRedeemCodeUsed)
	...
	_, err = s.client.RedeemCode.UpdateOneID(codeB.ID).
		...
		Save(s.ctx)
	s.Require().NoError(err)
	s.Require().NoError(s.repo.Use(s.ctx, codeA.ID, user.ID, 10.0), "Use codeA")
