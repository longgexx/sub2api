...
watch(
  ...
  (newAccount) => {
    if (newAccount) {
      ...
      // Load schedule settings (Anthropic OAuth/SetupToken only)
      loadScheduleSettings(newAccount)
...
function loadScheduleSettings(account: Account) {
  // Reset state
  ...
  scheduleEnabled.value = account.schedule_enabled ?? false
  scheduleTimezone.value = account.schedule_timezone ?? 'Asia/Shanghai'

  if (Array.isArray(account.schedule_rules)) {
    scheduleRules.value = account.schedule_rules.map((rule) => ({
      weekdays: [...rule.weekdays],
      start_time: minutesToTime(rule.start_minute),
      end_time: minutesToTime(rule.end_minute)
    }))
  } else {
    scheduleRules.value = []
  }
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'anthropic' && (props.account.type === 'oauth' || props.account.type === 'setup-token')) {
      ...
      // Schedule settings (top-level fields, not in extra)
      if (!applyScheduleConfig(updatePayload)) {
        submitting.value = false
        return
      }
    }
